/*! ng-formio v2.9.2 | https://unpkg.com/ng-formio@2.9.2/LICENSE.txt */
    ...
    controller: [
      ...
      function(
        ...
        $scope.checkErrors = function(form) {
          ...
          $scope.$broadcast('formioCheckErrors', form);
        ...
        $scope.onSubmit = function(form) {
          ...
          var showSuccess = function(submission, message) {
            if (message) {
              $scope.showAlerts({
                type: 'success',
                message: message
          ...
          var onSubmitDone = function(method, submission) {
            if ($scope.storageKey && !$scope.readOnly) {
              localStorage.setItem($scope.storageKey, '');
            }
            var message = '';
            ...
            showSuccess(submission, message);
            // Trigger the form submission.
            $scope.$emit('formSubmission', submission);
          };
          ...
          if ($scope.formAction) {
            ...
            $http[method]($scope.formAction, submissionData).then(function(response) {
              Formio.clearCache();
      ...
      controller: [
        ...
        function(
          ...
          $scope.findForm = function() {
            var start = $scope.$parent;

            while (start.form == null || start.form.components == null) {
              start = start.$parent;
            }
          ...
          // Pass through since this is an isolate scope.
          $scope.conditionContext = FormioUtils.buildConditionContext($scope.findForm(), $scope);

          var whenConditions = $scope.component.whenConditions;
          var conditions = formioComponents.conditions;
          if (!$scope.builder && whenConditions && whenConditions.length > 0) {
            $scope.$watch('data[' + JSON.stringify($scope.component.key)  +']' , function(newValue, oldValue) {
              whenConditions.map(function(condition) {
                var type = conditions[condition.type];
                var fulLNewValue = {};
                fulLNewValue[$scope.component.key] = angular.copy(newValue);
                var fullOldValue = {};
                fullOldValue[$scope.component.key] = oldValue;

                if (type != null) {
                  if (angular.equals(fulLNewValue, condition.value)) {
                    type.set($scope.conditionContext, condition, fulLNewValue, fullOldValue);
                  }
                  else {
                    type.unset($scope.conditionContext, condition, fulLNewValue, fullOldValue);
                  }
                }
              });
            }, true);
...
module.exports = function() {
  return {
    restrict: 'E',
    replace: true,
    templateUrl: 'formio-single.html',
    scope: {
      form: '=',
      formio: '=?',
      onSubmit: '=?',
    ...
    controller: [
      ...
      function(
        ...
        $scope.$watch('formioForm', function(newValue) {
          $scope.$emit('formioValueUpdate', newValue);
        });

        // Create the formio object.
        $scope.formio = $scope.formio || FormioScope.register($scope, $element, {
          form: true,
          submission: true
        });

        $scope.$on('formioCheckErrors', function() {
          $scope.checkErrors($scope.formioForm);
        });
        ...
        // Called when the form is submitted.
        $scope.onSelfSubmit = function(form) {
          $scope.onSubmit(form);
        };
      }
    ]
  ...
  return {
    ...
    controller: [
      ...
      function(
        ...
        var formValue = {};
        var nextPage = null;
        $scope.$on('formioValueUpdate', function(e, newValue) {
          formValue = newValue;
        });

        $scope.$on('formioSetNextPage', function(e, newPage) {
          nextPage = newPage;
        });

        $scope.$on('formioResetNextPage', function(e) {
          nextPage = null;
        });
        ...
        $scope.submit = function() {
          if ($scope.checkErrors(getForm())) {
            return;
          }

          $scope.onSubmit(getForm());
        };
        ...
        $scope.next = function() {
          ...
          if (nextPage) {
            var page = $scope.pages.findIndex(function(page) {
              return page.key === nextPage;
            });
...
module.exports = function() {
  var self = {
    buildConditionContext: function(form, $scope) {
      return {
        pages: self.getPages(form),

        setNextPage: function(page) {
          $scope.$emit('formioSetNextPage', page);
        },

        resetNextPage: function() {
          $scope.$emit('formioResetNextPage');
        }

      };
    },

    getPages: function(form) {
      var pages = [];
      form.components.forEach(function(component, index) {
...
},{"formio-utils":2}],60:[function(_dereq_,module,exports){
"use strict";
...
},{"lodash.filter":9,"lodash.get":10}],61:[function(_dereq_,module,exports){
"use strict";
module.exports = [
  '$q',
  '$rootScope',
...
app.directive('formioSingle', _dereq_('./directives/formioSingle'));
...
app.run([
  ...
  function($templateCache) {
    ...
    $templateCache.put('formio.html',
      "<div>\n  <i style=\"font-size: 2em;\" ng-if=\"formLoading\" ng-class=\"{'formio-hidden': !formLoading}\" class=\"formio-loading glyphicon glyphicon-refresh glyphicon-spin\"></i>\n  <div ng-repeat=\"alert in formioAlerts track by $index\" class=\"alert alert-{{ alert.type }}\" role=\"alert\" ng-if=\"::!builder\">\n      {{ alert.message | formioTranslate:null:builder }}\n  </div>\n  <formio-wizard ng-if=\"form.display === 'wizard'\" formio=\"formio\" form=\"form\" form-value=\"formValue\" on-submit=\"onSubmit\" submission=\"submission\" read-only=\"readOnly\" hide-components=\"hideComponents\" disable-components=\"disableComponents\" formio-options=\"formioOptions\" storage=\"form.name\"></formio-wizard>\n  <formio-single ng-if=\"form.display !== 'wizard'\" formio=\"formio\" form=\"form\" form-value=\"formValue\" on-submit=\"onSubmit\" submission=\"submission\" read-only=\"readOnly\" hide-components=\"hideComponents\" disable-components=\"disableComponents\" formio-options=\"formioOptions\" storage=\"form.name\"></formio-single>\n</div>\n"
    );
...
app.config([
  ...
  function(
    ...
    formioComponentsProvider.registerCondition({
      id: 'set-next-page',
      label: 'Set Next Page',
      template: '<br>Next Page:<select class="form-control input-md" ng-model="condition.targetPage" ng-options="page.key as page.title for page in pages"></select>',
      set: function(context, condition) {
        context.setNextPage(condition.targetPage);
      },
      unset: function(context) {
        context.resetNextPage();
      }
    });
  }
]);

},{"./components":31,"./directives/customValidator":47,"./directives/formio":48,"./directives/formioComponent":49,"./directives/formioComponentView":50,"./directives/formioDelete":51,"./directives/formioElement":52,"./directives/formioErrors":53,"./directives/formioSingle":54,"./directives/formioSubmission":55,"./directives/formioSubmissions":56,"./directives/formioWizard":57,"./factories/FormioScope":58,"./factories/FormioUtils":59,"./factories/Lodash":60,"./factories/formioInterceptor":61,"./factories/formioTableView":62,"./filters/flattenComponents":63,"./filters/safehtml":64,"./filters/tableComponents":65,"./filters/tableFieldView":66,"./filters/tableView":67,"./filters/translate":68,"./polyfills/polyfills":71,"./providers/Formio":72}],70:[function(_dereq_,module,exports){
"use strict";
'use strict';
